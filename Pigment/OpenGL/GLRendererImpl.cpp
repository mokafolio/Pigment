...
static_assert((Size)ReadFormat::Count == sizeof(s_glReadFormats) / sizeof(s_glReadFormats[0]),
              "ReadFormat mapping is not complete!");
...
static_assert((Size)BufferDataType::Count == sizeof(s_glDataTypes) / sizeof(s_glDataTypes[0]),
...
static_assert((Size)BufferUsage::Count == sizeof(s_glBufferUsage) / sizeof(s_glBufferUsage[0]),
              "BufferUsage mapping is not complete!");
...
static_assert((Size)PixelDataType::Count ==
                  sizeof(s_glPixelDataType) / sizeof(s_glPixelDataType[0]),
              "PixelDataType mapping is not complete!");
    ...
    m_alloc(&_alloc)
{
    ...
    m_glProgramUniformMatrix3fv =
        (PFNGLPROGRAMUNIFORMMATRIX3FVPROC)getGLFunctionPointer("glProgramUniformMatrix3fv");
    m_glProgramUniformMatrix4fv =
        (PFNGLPROGRAMUNIFORMMATRIX4FVPROC)getGLFunctionPointer("glProgramUniformMatrix4fv");
...
{
    auto it = findIf(m_commandBuffers.begin(),
                     m_commandBuffers.end(),
                     [&](const UniquePtr<CommandBuffer> & _buf) { return _buf.get() == &_buffer; });
...
TextureHandleResult RendererImpl::createTexture()
{
    ...
    return m_textures.append(
        { handle, 0, 0, 0, GL_TEXTURE_2D, TextureFormat::RGBA8, 0, RenderBufferHandle() });
}
...
static Error validateFrameBuffer()
{
    ...
    case GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT:
        return Error(ec::BadRenderBuffer,
    ...
    case GL_FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER:
        return Error(
            ec::BadRenderBuffer, "GL_FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER!", STICK_FILE, STICK_LINE);
    case GL_FRAMEBUFFER_INCOMPLETE_READ_BUFFER:
        return Error(
            ec::BadRenderBuffer, "GL_FRAMEBUFFER_INCOMPLETE_READ_BUFFER!", STICK_FILE, STICK_LINE);
    }
...
RenderBufferResult RendererImpl::createRenderBuffer(const RenderBufferSettings & _settings)
{
    ...
    for (const auto & rt : _settings.renderTargets)
    {
        ...
        TextureHandle th = m_textures.append(
            { texHandle, _settings.width, _settings.height, 0, GL_TEXTURE_2D, rt.format, 0, rbh });
...
void RendererImpl::destroyRenderBuffer(const RenderBufferHandle & _renderBufferHandle,
...
ProgramHandleResult RendererImpl::createProgram(const ProgramSettings & _settings)
{
    ...
    {
        ASSERT_NO_GL_ERROR(
            glBindAttribLocation(program, (*sit).m_location, (*sit).m_name.cString()));
    }
...
MeshHandleResult RendererImpl::createMesh(const MeshSettings & _settings)
{
    ...
    for (const VertexBufferHandle & vbh : _settings.vertexBuffers)
    {
        ...
        for (const auto & el : vb.layout.elements())
        {
            ...
            {
                ASSERT_NO_GL_ERROR(
                    glVertexAttribPointer(el.m_location,
                                          (UInt32)el.m_elementCount,
                                          s_glDataTypes[static_cast<Size>(el.m_type)],
                                          GL_FALSE,
                                          (UInt32)el.m_stride,
                                          ((char *)0 + el.m_offset)));
                ...
                {
                    ASSERT_NO_GL_ERROR(glVertexAttribDivisor((UInt32)el.m_location,
                                                             (UInt32)el.m_numberOfInstances));
                }
            }
            // 4X4 Matrix
            else if (el.m_elementCount == 16 && el.m_numberOfInstances > 0)
            {
                ...
                ASSERT_NO_GL_ERROR(glVertexAttribPointer(el.m_location,
                                                         4,
                                                         dataType,
                                                         GL_FALSE,
                                                         (UInt32)el.m_stride,
                                                         ((char *)0 + el.m_offset)));
                ASSERT_NO_GL_ERROR(glVertexAttribPointer(
                    el.m_location + 1,
                    4,
                    dataType,
                    GL_FALSE,
                    (UInt32)el.m_stride,
                    ((char *)0 + el.m_offset +
                     s_bufferDataTypeInfos[static_cast<Size>(el.m_type)].byteCount * 4)));
                ASSERT_NO_GL_ERROR(glVertexAttribPointer(
                    el.m_location + 2,
                    4,
                    dataType,
                    ...
                    (UInt32)el.m_stride,
                    ((char *)0 + el.m_offset +
                     s_bufferDataTypeInfos[static_cast<Size>(el.m_type)].byteCount * 8)));
                ASSERT_NO_GL_ERROR(glVertexAttribPointer(
                    el.m_location + 3,
                    4,
                    dataType,
                    ...
                    (UInt32)el.m_stride,
                    ((char *)0 + el.m_offset +
                     s_bufferDataTypeInfos[static_cast<Size>(el.m_type)].byteCount * 12)));
            ...
            {
                // Unsupported
                err = Error(ec::UnsupportedRenderFeature,
                            String::concat("The vertex element named ",
                                           el.m_name,
                                           " has an unsupported element count or configuration"),
                            STICK_FILE,
                            STICK_LINE);
                break;
            }
...
VertexBufferHandleResult RendererImpl::createVertexBuffer(const VertexLayout & _layout,
                                                          BufferUsage _usage)
{
    GLuint glHandle;
...
static void bindFrameBuffer(GLRenderBuffer & _rb, bool _bMarkDirty)
{
    ...
    ASSERT_NO_GL_ERROR(
        glDrawBuffers((GLuint)_rb.colorAttachmentPoints.count(), &_rb.colorAttachmentPoints[0]));
}
...
Error RendererImpl::submit()
{
    ...
    for (const CommandBufferImpl * buf : m_commandBufferQueue)
    {
        for (const auto & cmd : buf->m_queue)
        {
            ...
            {
                const CommandBufferImpl::BindRenderBufferCommand & brcmd =
                    cmd.command.bindRenderBufferCommand;
            ...
            else if (cmd.type == CommandBufferImpl::CommandType::LoadPixels)
            {
                ...
                tex.glType = type;
                GLenum glDataType =
                    s_glPixelDataType[static_cast<Size>(cmd.command.loadPixelsCommand.dataType)];
                const GLTextureFormat & format =
                    s_glTextureFormats[static_cast<Size>(cmd.command.loadPixelsCommand.format)];

                ASSERT_NO_GL_ERROR(glActiveTexture(GL_TEXTURE0));
                ASSERT_NO_GL_ERROR(glBindTexture(type, tex.glHandle));
                tex.format = cmd.command.loadPixelsCommand.format;
                ASSERT_NO_GL_ERROR(
                    glPixelStorei(GL_UNPACK_ALIGNMENT, cmd.command.loadPixelsCommand.alignment));
                ...
                if (type == GL_TEXTURE_1D)
                {
                    ASSERT_NO_GL_ERROR(
                        glTexImage1D(type,
                                     0,
                                     format.glInternalFormat,
                                     static_cast<GLuint>(cmd.command.loadPixelsCommand.width),
                                     0,
                                     format.glFormat,
                                     glDataType,
                                     cmd.command.loadPixelsCommand.dataSource));
                }
                else if (type == GL_TEXTURE_2D)
                {
                    ASSERT_NO_GL_ERROR(
                        glTexImage2D(type,
                                     ...
                                     format.glInternalFormat,
                                     static_cast<GLuint>(cmd.command.loadPixelsCommand.width),
                                     static_cast<GLuint>(cmd.command.loadPixelsCommand.height),
                                     0,
                                     format.glFormat,
                                     glDataType,
                                     cmd.command.loadPixelsCommand.dataSource));
                ...
                else if (type == GL_TEXTURE_3D)
                {
                    ASSERT_NO_GL_ERROR(
                        glTexImage3D(type,
                                     ...
                                     static_cast<GLuint>(cmd.command.loadPixelsCommand.width),
                                     static_cast<GLuint>(cmd.command.loadPixelsCommand.height),
                                     static_cast<GLuint>(cmd.command.loadPixelsCommand.depth),
                                     0,
                                     format.glFormat,
                                     glDataType,
                                     cmd.command.loadPixelsCommand.dataSource));
                ...
                {
                    // make sure there is now sampler bound as that would overwrite
                    // GL_TEXTURE_MAX_LEVEL
                    ASSERT_NO_GL_ERROR(glBindSampler(0, 0));
                    ASSERT_NO_GL_ERROR(
                        glTexParameteri(type, GL_TEXTURE_MAX_LEVEL, tex.mipmapLevelCount));
            ...
            {
                const CommandBufferImpl::UpdatePixelsCommand & upcmd =
                    cmd.command.updatePixelsCommand;
                const GLTexture & tex = m_textures.get(upcmd.handle);
                GLenum glDataType =
                    s_glPixelDataType[static_cast<Size>(cmd.command.updatePixelsCommand.dataType)];
                ...
                {
                    ASSERT_NO_GL_ERROR(glTexSubImage1D(tex.glType,
                                                       0,
                                                       static_cast<GLuint>(upcmd.x),
                                                       static_cast<GLuint>(upcmd.width),
                                                       format.glFormat,
                                                       glDataType,
                                                       upcmd.dataSource));
                }
                ...
                {
                    ASSERT_NO_GL_ERROR(glTexSubImage2D(tex.glType,
                                                       0,
                                                       static_cast<GLuint>(upcmd.x),
                                                       static_cast<GLuint>(upcmd.y),
                                                       static_cast<GLuint>(upcmd.width),
                                                       static_cast<GLuint>(upcmd.height),
                                                       format.glFormat,
                                                       glDataType,
                                                       upcmd.dataSource));
                }
                ...
                {
                    ASSERT_NO_GL_ERROR(glTexSubImage3D(tex.glType,
                                                       0,
                                                       static_cast<GLuint>(upcmd.x),
                                                       static_cast<GLuint>(upcmd.y),
                                                       static_cast<GLuint>(upcmd.z),
                                                       static_cast<GLuint>(upcmd.width),
                                                       static_cast<GLuint>(upcmd.height),
            ...
            {
                const CommandBufferImpl::SetProgramVariableCommand & spvcmd =
                    cmd.command.setProgramVariableCommand;
                ...
                if (mit != program.uniformMap.end())
                {
                    ...
                    {
                        ASSERT_NO_GL_ERROR(m_glProgramUniform1f(
                            program.glHandle, var.glLoc, spvcmd.value.floatValue));
                    }
                    else if (var.type == ProgramVariableType::Vec2f)
                    {
                        ASSERT_NO_GL_ERROR(m_glProgramUniform2fv(
                            program.glHandle, var.glLoc, 1, &spvcmd.value.floatValue));
                    }
                    else if (var.type == ProgramVariableType::Vec3f)
                    {
                        ASSERT_NO_GL_ERROR(m_glProgramUniform3fv(
                            program.glHandle, var.glLoc, 1, &spvcmd.value.floatValue));
                    }
                    else if (var.type == ProgramVariableType::Vec4f)
                    {
                        ASSERT_NO_GL_ERROR(m_glProgramUniform4fv(
                            program.glHandle, var.glLoc, 1, &spvcmd.value.floatValue));
                    }
                    else if (var.type == ProgramVariableType::Matrix3f)
                    {
                        ASSERT_NO_GL_ERROR(m_glProgramUniformMatrix3fv(
                            program.glHandle, var.glLoc, 1, false, &spvcmd.value.floatValue));
                    }
                    else if (var.type == ProgramVariableType::Matrix4f)
                    {
                        ASSERT_NO_GL_ERROR(m_glProgramUniformMatrix4fv(
                            program.glHandle, var.glLoc, 1, false, &spvcmd.value.floatValue));
                    }
                    else
                    {
                        ret = Error(ec::BadProgramVariable,
                ...
                {
                    ret = Error(ec::BadProgramVariable,
                                "The program variable was not found",
                                STICK_FILE,
                                STICK_LINE);
                    break;
                }
            ...
            {
                const CommandBufferImpl::LoadVertexDataCommand & lvdcmd =
                    cmd.command.loadVertexDataCommand;
                const GLVertexBuffer & vb = m_vertexBuffers.get(lvdcmd.vertexBuffer);
                ASSERT_NO_GL_ERROR(glBindBuffer(GL_ARRAY_BUFFER, vb.glHandle));
                ASSERT_NO_GL_ERROR(glBufferData(GL_ARRAY_BUFFER,
                                                lvdcmd.byteCount,
                                                lvdcmd.dataSource,
                                                s_glBufferUsage[static_cast<Size>(vb.usage)]));
            }
            else if (cmd.type == CommandBufferImpl::CommandType::UpdateVertexData)
            {
                const CommandBufferImpl::UpdateVertexDataCommand & uvdcmd =
                    cmd.command.updateVertexDataCommand;
                const GLVertexBuffer & vb = m_vertexBuffers.get(uvdcmd.vertexBuffer);
                ASSERT_NO_GL_ERROR(glBindBuffer(GL_ARRAY_BUFFER, vb.glHandle));
                ASSERT_NO_GL_ERROR(glBufferSubData(
                    GL_ARRAY_BUFFER, uvdcmd.byteOffset, uvdcmd.byteCount, uvdcmd.dataSource));
            }
            else if (cmd.type == CommandBufferImpl::CommandType::LoadIndexData)
            {
                const CommandBufferImpl::LoadIndexDataCommand & lidcmd =
                    cmd.command.loadIndexDataCommand;
                const GLIndexBuffer & ib = m_indexBuffers.get(lidcmd.indexBuffer);
                ASSERT_NO_GL_ERROR(glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ib.glHandle));
                ASSERT_NO_GL_ERROR(glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                                                lidcmd.byteCount,
                                                lidcmd.dataSource,
                                                s_glBufferUsage[static_cast<Size>(ib.usage)]));
            }
            else if (cmd.type == CommandBufferImpl::CommandType::UpdateIndexData)
            {
                const CommandBufferImpl::UpdateIndexDataCommand & uidcmd =
                    cmd.command.updateIndexDataCommand;
                const GLIndexBuffer & ib = m_indexBuffers.get(uidcmd.indexBuffer);
                ASSERT_NO_GL_ERROR(glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ib.glHandle));
                ASSERT_NO_GL_ERROR(glBufferSubData(GL_ELEMENT_ARRAY_BUFFER,
                                                   uidcmd.byteOffset,
                                                   uidcmd.byteCount,
                                                   uidcmd.dataSource));
            }
            ...
            {
                const CommandBufferImpl::SetViewportCommand & svcmd =
                    cmd.command.setViewportCommand;
            ...
            {
                const CommandBufferImpl::ClearBuffersCommand & cbcmd =
                    cmd.command.clearBuffersCommand;
                GLuint buffers = 0;
                if (static_cast<Size>(cbcmd.buffersMask) & static_cast<Size>(BufferType::Color))
                {
                    ...
                    ASSERT_NO_GL_ERROR(glClearColor(cbcmd.settings.clearColor.r,
                                                    cbcmd.settings.clearColor.g,
                                                    cbcmd.settings.clearColor.b,
                                                    cbcmd.settings.clearColor.a));
                }
                ...
                if (static_cast<Size>(cbcmd.buffersMask) & static_cast<Size>(BufferType::Stencil))
                {
                    ...
                    ASSERT_NO_GL_ERROR(
                        glStencilMaskSeparate(GL_FRONT, cbcmd.settings.stencilFrontFaceWriteMask));
                    ASSERT_NO_GL_ERROR(
                        glStencilMaskSeparate(GL_BACK, cbcmd.settings.stencilBackFaceWriteMask));
                }
            ...
            {
                const CommandBufferImpl::AttachTextureCommand & atcmd =
                    cmd.command.attachTextureCommand;
                ...
                if (index == (Size)-1)
                {
                    ...
                    ret = Error(ec::UnspecifiedError,
                                "All texture units are allready in use",
                                STICK_FILE,
                                STICK_LINE);
                    break;
                }

                ASSERT_NO_GL_ERROR(m_glProgramUniform1i(
                    program.glHandle, program.uniforms[uit->value].glLoc, (GLint)index));

                // the next draw call should rebind the program/tetures, so we invalidate the last
                // handle here.
                lastProgram = ProgramHandle();
            }
            else if (cmd.type == CommandBufferImpl::CommandType::Draw)
            {
                    ...
                    for (Size i = 0; i < program.textureUnits.count(); ++i)
                    {
                        if (program.textureUnits[i].uniformIndex != -1)
                        {
                            ...
                            const GLSampler & sampler =
                                m_samplers.get(program.textureUnits[i].sampler);
                ...
                if (lastRenderState != dcmd.renderState)
                {
                    ...
                    if (lastRenderState.isValid())
                    {
                        ...
                        stencilDiffMask = renderState.settings.stencilDifferenceMask(
                            lp.settings.stencilState().state);
                        bStencilBackFaceRefDirty =
                            renderState.settings.stencilState().backFaceRef !=
                            lp.settings.stencilState().backFaceRef;
                        bStencilBackFaceMaskDirty =
                            renderState.settings.stencilState().backFaceMask !=
                            lp.settings.stencilState().backFaceMask;
                        bStencilFrontFaceRefDirty =
                            renderState.settings.stencilState().frontFaceRef !=
                            lp.settings.stencilState().frontFaceRef;
                        bStencilFrontFaceMaskDirty =
                            renderState.settings.stencilState().frontFaceMask !=
                            lp.settings.stencilState().frontFaceMask;
                        bStencilFrontFaceWriteMaskDirty =
                            renderState.settings.stencilState().frontFaceWriteMask !=
                            lp.settings.stencilState().frontFaceWriteMask;
                        bStencilBackFaceWriteMaskDirty =
                            renderState.settings.stencilState().backFaceWriteMask !=
                            lp.settings.stencilState().backFaceWriteMask;
                    }
                    ...
                    if (RenderStateSettings::isFlagDifferent(diffMask,
                                                             RenderFlag::RF_Multisampling))
                    {
                    ...
                    {
                        ASSERT_NO_GL_ERROR(
                            glDepthMask(renderState.settings.isFlagSet(RF_DepthWrite)));
                    }

                    if (RenderStateSettings::isFieldDifferent(diffMask,
                                                              RenderFlag::RF_DepthFuncMask))
                    {
                        ASSERT_NO_GL_ERROR(glDepthFunc(s_glCompareFuncs[static_cast<Size>(
                            renderState.settings.depthFunction())]));
                    }

                    if (RenderStateSettings::isFlagDifferent(diffMask,
                                                             RenderFlag::RF_ColorWriteRed) ||
                        RenderStateSettings::isFlagDifferent(diffMask,
                                                             RenderFlag::RF_ColorWriteGreen) ||
                        RenderStateSettings::isFlagDifferent(diffMask,
                                                             RenderFlag::RF_ColorWriteBlue) ||
                        RenderStateSettings::isFlagDifferent(diffMask,
                                                             RenderFlag::RF_ColorWriteAlpha))
                    {
                        ASSERT_NO_GL_ERROR(glColorMask(
                            renderState.settings.isFlagSet(RenderFlag::RF_ColorWriteRed),
                    ...
                    if (RenderStateSettings::isFieldDifferent(diffMask,
                                                              RenderFlag::RF_ColorBlendModeMask) ||
                        RenderStateSettings::isFieldDifferent(diffMask,
                                                              RenderFlag::RF_AlphaBlendModeMask))
                    {
                        ASSERT_NO_GL_ERROR(
                            glBlendEquationSeparate(s_glBlendModes[static_cast<Size>(
                                                        renderState.settings.colorBlendMode())],
                                                    s_glBlendModes[static_cast<Size>(
                                                        renderState.settings.alphaBlendMode())]));
                    }

                    if (RenderStateSettings::isFieldDifferent(
                            diffMask, RenderFlag::RF_ColorSourceBlendFuncMask) ||
                        RenderStateSettings::isFieldDifferent(
                            diffMask, RenderFlag::RF_ColorDestBlendFuncMask) ||
                        RenderStateSettings::isFieldDifferent(
                            diffMask, RenderFlag::RF_AlphaSourceBlendFuncMask) ||
                        RenderStateSettings::isFieldDifferent(
                            diffMask, RenderFlag::RF_AlphaDestBlendFuncMask))
                    {
                        ASSERT_NO_GL_ERROR(glBlendFuncSeparate(
                            s_glBlendFuncs[static_cast<Size>(
                                renderState.settings.colorSourceBlendFunction())],
                            s_glBlendFuncs[static_cast<Size>(
                                renderState.settings.colorDestBlendFunction())],
                            s_glBlendFuncs[static_cast<Size>(
                                renderState.settings.alphaSourceBlendFunction())],
                            s_glBlendFuncs[static_cast<Size>(
                                renderState.settings.alphaDestBlendFunction())]));
                    }

                    if (RenderStateSettings::isFlagDifferent(diffMask, RF_FrontFaceClockwise))
                    {
                        ASSERT_NO_GL_ERROR(glFrontFace(
                            renderState.settings.isFlagSet(RF_FrontFaceClockwise) ? GL_CW
                                                                                  : GL_CCW));
                    }

                    if (RenderStateSettings::isFieldDifferent(diffMask,
                                                              RenderFlag::RF_CullFaceMask))
                    {
                        if (renderState.settings.cullFace() != FaceType::None)
                        {
                            ASSERT_NO_GL_ERROR(glEnable(GL_CULL_FACE));
                            ASSERT_NO_GL_ERROR(glCullFace(
                                s_glFaceType[static_cast<Size>(renderState.settings.cullFace())]));
                        }
                        else
                        {
                    ...
                    if (renderState.settings.stencilState().bWriteSeparate)
                    {
                        ...
                        {
                            ASSERT_NO_GL_ERROR(glStencilMaskSeparate(
                                GL_BACK, renderState.settings.stencilState().backFaceWriteMask));
                        }

                        if (bStencilFrontFaceWriteMaskDirty)
                        {
                            ASSERT_NO_GL_ERROR(glStencilMaskSeparate(
                                GL_FRONT, renderState.settings.stencilState().frontFaceWriteMask));
                        }
                    }
                    else
                    {
                        if (bStencilFrontFaceWriteMaskDirty)
                        {
                            ASSERT_NO_GL_ERROR(glStencilMask(
                                renderState.settings.stencilState().frontFaceWriteMask));
                        }
                    }
                    ...
                    {
                        if (RenderStateSettings::isFieldDifferent(stencilDiffMask,
                                                                  SF_StencilFuncBackFaceMask) ||
                            bStencilBackFaceRefDirty || bStencilBackFaceMaskDirty)
                        {
                            ASSERT_NO_GL_ERROR(glStencilFuncSeparate(
                                GL_BACK,
                                s_glCompareFuncs[static_cast<Size>(
                                    renderState.settings.stencilFunctionBackFace())],
                                renderState.settings.stencilState().backFaceRef,
                                renderState.settings.stencilState().backFaceMask));
                        }

                        if (RenderStateSettings::isFieldDifferent(stencilDiffMask,
                                                                  SF_StencilFuncFrontFaceMask) ||
                            bStencilFrontFaceRefDirty || bStencilFrontFaceMaskDirty)
                        {
                            ASSERT_NO_GL_ERROR(glStencilFuncSeparate(
                                GL_FRONT,
                                s_glCompareFuncs[static_cast<Size>(
                                    renderState.settings.stencilFunctionFrontFace())],
                                renderState.settings.stencilState().frontFaceRef,
                                renderState.settings.stencilState().frontFaceMask));
                        }
                    }
                    else
                    {
                        ...
                        {
                            ASSERT_NO_GL_ERROR(
                                glStencilFunc(s_glCompareFuncs[static_cast<Size>(
                                                  renderState.settings.stencilFunctionFrontFace())],
                                              renderState.settings.stencilState().frontFaceRef,
                                              renderState.settings.stencilState().frontFaceMask));
                        }
                    ...
                    {
                        if (RenderStateSettings::isFieldDifferent(stencilDiffMask,
                                                                  SF_StencilOpSFailFrontFaceMask) ||
                            RenderStateSettings::isFieldDifferent(
                                stencilDiffMask, SF_StencilOpDPFailFrontFaceMask) ||
                            RenderStateSettings::isFieldDifferent(stencilDiffMask,
                                                                  SF_StencilOpDPPassFrontFaceMask))
                        {
                            ASSERT_NO_GL_ERROR(glStencilOpSeparate(
                                GL_FRONT,
                                s_glStencilOps[static_cast<Size>(
                                    renderState.settings.stencilSFailOperationFrontFace())],
                                s_glStencilOps[static_cast<Size>(
                                    renderState.settings.stencilDPFailOperationFrontFace())],
                                s_glStencilOps[static_cast<Size>(
                                    renderState.settings.stencilDPPassOperationFrontFace())]));
                        }

                        if (RenderStateSettings::isFieldDifferent(stencilDiffMask,
                                                                  SF_StencilOpSFailBackFaceMask) ||
                            RenderStateSettings::isFieldDifferent(stencilDiffMask,
                                                                  SF_StencilOpDPFailBackFaceMask) ||
                            RenderStateSettings::isFieldDifferent(stencilDiffMask,
                                                                  SF_StencilOpDPPassBackFaceMask))
                        {
                            ASSERT_NO_GL_ERROR(glStencilOpSeparate(
                                GL_BACK,
                                s_glStencilOps[static_cast<Size>(
                                    renderState.settings.stencilSFailOperationBackFace())],
                                s_glStencilOps[static_cast<Size>(
                                    renderState.settings.stencilDPFailOperationBackFace())],
                                s_glStencilOps[static_cast<Size>(
                                    renderState.settings.stencilDPPassOperationBackFace())]));
                        }
                    }
                    else
                    {
                        if (RenderStateSettings::isFieldDifferent(stencilDiffMask,
                                                                  SF_StencilOpSFailFrontFaceMask))
                        {
                            ASSERT_NO_GL_ERROR(glStencilOp(
                                s_glStencilOps[static_cast<Size>(
                                    renderState.settings.stencilSFailOperationFrontFace())],
                                s_glStencilOps[static_cast<Size>(
                                    renderState.settings.stencilDPFailOperationFrontFace())],
                ...
                if (dcmd.instanceCount == 1)
                {
                    ...
                    {
                        ASSERT_NO_GL_ERROR(glDrawArrays(
                            s_glVertexDrawModes[static_cast<Size>(dcmd.vertexDrawMode)],
                    ...
                    else
                    {
                        ...
                        ASSERT_NO_GL_ERROR(glDrawElements(
                            s_glVertexDrawModes[static_cast<Size>(dcmd.vertexDrawMode)],
                            static_cast<GLuint>(dcmd.vertexCount),
                            s_glDataTypes[bdtindex],
                            (void *)(dcmd.vertexOffset *
                                     s_bufferDataTypeInfos[bdtindex].byteCount)));
                    }
                }
                else
                {
                    if (!mesh.indexBuffer.isValid())
                    {
                        ASSERT_NO_GL_ERROR(glDrawArraysInstanced(
                            s_glVertexDrawModes[static_cast<Size>(dcmd.vertexDrawMode)],
                            static_cast<GLuint>(dcmd.vertexOffset),
                            static_cast<GLuint>(dcmd.vertexCount),
                    ...
                    else
                    {
                        const GLIndexBuffer & buf = m_indexBuffers.get(mesh.indexBuffer);
                        Size bdtindex = static_cast<Size>(buf.dataType);
                        ASSERT_NO_GL_ERROR(glDrawElementsInstanced(
                            s_glVertexDrawModes[static_cast<Size>(dcmd.vertexDrawMode)],
                            static_cast<GLuint>(dcmd.vertexCount),
                            s_glDataTypes[bdtindex],
