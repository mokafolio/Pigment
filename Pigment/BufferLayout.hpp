...
namespace pigment
{
    struct STICK_API BufferElement
    {
        ...
        //if 0, the element is set for each vertex, if 1, the element is set for each instance, if 2 the element is
        //set for every two instances etc. (only used by VertexLayout)
        stick::Size m_numberOfInstances;

        BufferElement(stick::Size _elementCount, stick::Size _stride,
                      const stick::String & _name, BufferDataType _dataType, stick::UInt32 _location, stick::Size _numberOfInstances):
            m_name(_name),
    ...
    public:
        ...
        BufferLayout & addElement(const stick::String & _name, stick::Size _elementCount, BufferDataType _dataType = BufferDataType::Float32);

        BufferLayout & addElement(const stick::String & _name, stick::Size _elementCount, stick::UInt32 _location, BufferDataType _dataType = BufferDataType::Float32);

        //needs to be called after all elements have been added to calculate some internalts (stride, offset of the elements)
        void finish();
    ...
    namespace detail
    {
        ...
        {
            _error = stick::ErrorReport(error::BadVertexBuffer, "Unsupported buffer data type!", __FILE__, __LINE__);
            return BufferDataType::UInt32; //just to return anything to satisfy the compiler, will be ignored
        }
