...
int main(int _argc, const char * _args[])
{
    ...
    if (window)
    {
        ...
        RenderBuffer rb = renderer.createRenderBuffer({800, 600, 4, {{TextureFormat::RGBA8, 0}}}).ensure();

        VertexLayout layoutTex;
        layoutTex.
        addElement("vertex", 2).
        addElement("textureCoordinates", 2).
        finish();

        // a very simple vertex shader for the triangle
        String vertexShaderTex = "#version 150 \n"
                                 "in vec2 vertex; \n"
        ...
        // a simple fragment shader for the triangle
        String fragmentShaderTex = "#version 150 \n"
        ...
        // create a vertex buffer with the layout
        VertexBufferHandle vb2 = renderer.createVertexBuffer(layoutTex, BufferUsage::StaticDraw).ensure();
        ...
        // load the vertex data for the triangle into the vertex buffer
        Float32 stripData[16] = { -0.5, -0.5,  0.0, 0.0,  -0.5, 0.5,  0.0, 1.0,  0.5, -0.5,  1.0, 0.0,  0.5, 0.5,  1.0, 1.0};
